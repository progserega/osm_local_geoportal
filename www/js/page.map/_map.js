$(function() {
  ...
  // add mouse pos:
  //L.control.mousePosition({lngFirst: true, prefix: 'Координаты указателя: ', separator: ' , ', position:'topleft', emptyString: 'Координаты недоступны'}).addTo(osm.map);
  ...
  if (!frame_map) {
    ...
    osm.map.addControl(new L.Control.OsmNotes({position:'topleft', linktitle: 'Сообщить об ошибке на карте' }));
...
//	.openOn(osm.map);
//    }
    osm.map.on('click', getInfoOnClick);
//----------------------------------
    ...
    // RemovedItems
    osm.removedItems = new L.Control.removedItems();
    $('#EditJOSM a').click($.proxy(osm.removedItems._click_link, osm.removedItems));
