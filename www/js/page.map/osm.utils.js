...
osm.opento = function() {
  ...
  //var point = L.CRS.EPSG900913.projection.unproject((new L.Point(x, y)).divideBy(earthRadius));
  var point = L.CRS.EPSG900913.projection.project((new L.LatLng(center.lat, center.lng))).multiplyBy(earthRadius);

  $('#opento-windyty')[0].href = 'https://www.windyty.com/?' + center.lat + ',' + center.lng + ',' + zoom;
  $('#opento-kadastre')[0].href = 'http://pkk5.rosreestr.ru/#' + 'x=' + point.x + '&y=' + point.y + '&z=' + zoom;
  $('#opento-osmdrsk')[0].href = 'http://osm.prim.drsk.ru/?' + 'lat=' + center.lat + '&lon=' + center.lng + '&zoom=' + zoom + '&layers=D'; 
...
osm.mapperMode.changeMode = function() {
  if (osm.mapperMode.on) {
    ...
    // RemovedItems:
    osm.map.addControl(osm.removedItems);
  }
  else {
    ...
    // RemovedItems:
    osm.map.removeControl(osm.removedItems);
  }
}
