...
osm.poi = {
  ...
  createPopupText: function(getdata) {
    ...
    var ref;
    if (!(getdata.ref==null)) {
      ref=$('<div>').text('Обозначение: '+ getdata.ref).addClass('ref');
    }
    ...
    if (!(getdata.wikipedia == null)) {
      if (getdata.wikipedia.indexOf(':') == 2) {
        ...
        wiki = $('<div>').addClass('main wiki')
          .append($('<a>').attr('href', wiki_link).text('Схема объекта'));
...
*/
	...
	var line_profile_link;
    if (!(getdata.osm_id == null) && (getdata.class_ru == "Опора ЛЭП" || getdata.class_ru == "Столб электропередачи")) {
        
        var profile_link = "http://tools.map.prim.drsk.ru/power_lines_profile/index.cgi?" + getdata.osm_id
        line_profile_link = $('<div>').addClass('tools_link')
          .append($('<a>').attr('href', profile_link).text('Профиль линий, проходящих по этой опоре'));
    }

  	var scheme;
    if (!(getdata.osm_id == null) && !(getdata.scheme == null)) {
        
      var scheme_link = getdata.scheme;
	  // проверяем, указан полный путь или только имя файла схемы:
      if (scheme_link.indexOf('://') < 0) {
        scheme_link = 'http://wiki.rs.int/lib/exe/fetch.php/изображения_для_карты:'+getdata.scheme
      }
      scheme = $('<div>').addClass('scheme_link')
          .append($('<a>').attr('href', scheme_link).text('Схема объекта'));
    }
        ...
        technical = technical.after($('<tr>')
          ...
          .append($('<td>').addClass('buttons')
            .append($('<a target="_blank">')
              .attr('href', 'http://osm.prim.drsk.ru/' + osm_id_type + '/' + osm_id_id)
              .append($('<img>').attr('src', '/img/popup/osm-info.png'))
            )
            .append($('<a target="_blank">')
              .attr('href', 'http://osm.prim.drsk.ru/' + osm_id_type + '/' + osm_id_id + '/history')
              .append($('<img>').attr('src', '/img/popup/osm-history.png'))
              .addClass('button_space')
            )
            .append($('<a target="_blank">')
              .attr('href', 'http://osm.prim.drsk.ru/edit?editor=id&' + osm_id_type + '=' + osm_id_id)
